...
blueprint! {
    struct CandyDex {
        collected_xrd: Vault,                                     // XRD vault.
        candy_vaults: HashMap<Address, Vault>,                    // Candy Hashmap of vaults.
        candy_map: HashMap<Address, (String, String, Decimal)>,   // Candy Hashmap with name, symbol, price.
        badge_map: HashMap<Address, (Decimal, Decimal, Address)>, // metaBadge Hashmap with entry fee level, metaCandy amount & Candy. 
        meta_map: HashMap<Address, (Decimal, Decimal, Address)>,  // Candy Hashmap with accrued fee, metaCandy amount & address.        
        meta: HashMap<Address, MetaToken>,                        // metaCandy Hashmap with MetaToken resource adresses. 
        minter_badge: Vault,                                      // Badge to mint and burn metaCandies.
        owner_badge: ResourceDef,                                 // Owner badge to determine protocol fee and collect accrued XRD fee. 
        xrd_fee: Decimal,                                         // Protocol XRD fee variable. 
        xrd_claimed: Decimal,                                     // Amount of accrued XRD protocol fee withdrawed by protocol owner. 
        fee: Decimal                                              // Protocol fee variable.
    }
    ...
    impl CandyDex {
        pub fn new(fee: Decimal) -> (Component,Bucket) {
            ...
            // Create a metaBadge and populate a hashmap to associate a determinated entry fee level to a candy stoke event.
            fn add_meta_badge(&mut self, symbol: String, candy_addr: Address, entry_fee: Decimal, meta_amnt: Decimal) -> Bucket {
            ...
            // Populate a hashmap to associate a determinated metaBadge address to a metaCandy amount.
            fn meta_amounts(&mut self, meta_badge: Bucket, meta_amount: Decimal, flag: Decimal) -> Bucket {
                ...
                if flag == 0i32.into() {
                  ...
                  } else { self.badge_map.insert(meta_badge_addr,(entry_fee,m_candy_amnt,candy_addr));
                    output_bucket = meta_badge; }
                } else { std::process::abort(); }
                output_bucket
            }
            // Create a metaCandy resource relative to a kind of candy provided to protocol by end users. 
            fn add_meta_candy(&mut self, candy_name: String, candy_symbol: String, candy_address: Address) -> Address {
                ...
                let (a,b,_c) = self.meta_map.get(&candy_address.clone()).unwrap();
                let accrued_fee = *a;
                let meta_minted = *b;
                self.meta_map.insert(candy_address,(accrued_fee,meta_minted,meta_address));
                meta_address
            }
            // Mint a metacandy amount relative to an amount of candy provided to protocol and update total metaCandy minted amount.
            ...
            // Burn a metacandy amount relative to amount of candy claimed by end user via "unstock_candy" function.
            fn meta_burn(&mut self, meta_candy: Bucket) {
                let temp_badge = self.minter_badge.take(1);
                meta_candy.burn_with_auth(temp_badge.present());
                self.minter_badge.put(temp_badge);
            }
            // Burn metaBadge once all relative metaCandy has been claimed by end user via "unstock_candy" function.
            ...
            // Put candy to sell in vault whenever end user specify an exact number of candy to sell using swap functions.
            fn candyput_pri(&mut self, prc_in: Decimal, prc_out: Decimal, addr_in: Address, candy: Bucket) -> (Decimal,Decimal) {
                ...
                if prc_in == 0i32.into() { candy_amnt = CandyDex::adjust_fee(self, candy_amnt); }   
                let amount = CandyDex::candy_sum(self, candy_amnt, candy.resource_address(), addr_in, 1i32.into());
                let v = self.candy_vaults.entry(candy.resource_address()).or_insert(Vault::new(candy.resource_address()));
                v.put(candy);
                (nmbr,amount)
            } 
            // Put candy to sell in vault whenever the number of candy to sell is determined by protocol.
            fn candyput_sec(&mut self, amnt: Decimal, addr: Address, prc_in: Decimal, prc_out: Decimal, candy: Bucket) -> Bucket {
                ...
                let amnt_: Decimal;
                if prc_in == 0i32.into() {amnt_ = CandyDex::candy_sum(self, amnt, addr, candy.resource_address(), 1i32.into());}                     
            ...
            fn candy_sum(&mut self, amnt_pri: Decimal, addr_pri: Address, addr_sec:Address, flag: Decimal)-> Decimal {     
                ...
                if flag == 1i32.into() {         
                  ...
                  let price_new: Decimal = CandyDex::price_mod(self, amount, addr_pri, price_in, 1i32.into());
        ...
        pub fn stock_candy(&mut self, candy: Bucket, new_price: Decimal, name: String, symbol: String) -> (Bucket,Bucket) {
            ...
            match self.candy_map.get(&candy_addr) {
                  Some((_a,_b,_c)) => { info!(" Candy already in Vault. Please use restock_candy function");
                                    std::process::abort()
                                  }
            ...
            let output_badge: Bucket = CandyDex::meta_amounts(self, meta_badge, meta_candy.amount().clone(), none);
        ...
        pub fn restock_candy(&mut self, candy: Bucket) -> (Bucket,Bucket) {
            ...
            let output_badge: Bucket = CandyDex::meta_amounts(self, meta_badge, meta_candy.amount().clone(), 0i32.into());
            (meta_candy,output_badge)
        }
            // Unstock candy function callable by an end user wishing to withdraw owned candy liquidity amount from protocol.
        pub fn unstock_candy(&mut self, candy_addr: Address, meta_candy: Bucket, meta_badge: Bucket) -> (Bucket,Bucket,Bucket) {
        ...
        pub fn stock_position(&mut self, meta_badge: BucketRef) -> BucketRef {
            ...
            match self.badge_map.get(&meta_badge.resource_address()) {
             Some((a,b,c)) => { let (accrued_fee,total_minted,_meta_address) = self.meta_map.get(&c).unwrap();
             ...
             None => {info!(" No badge's correspondence! ");       
                      std::process::abort()
                     } 
            }
            meta_badge   
        ...
        pub fn get_reserve(&self, candy_addr: Address) {
            match self.candy_map.get(&candy_addr) {
             Some((a,_b,_c)) => { let total_candy = self.candy_vaults.get(&candy_addr).unwrap().amount();
        ...
        pub fn get_candy_sell_amount_bexsc(&mut self, candy_addr: Address, xrd_amnt: Decimal) -> Decimal {     
        ...
        pub fn get_xrd_buy_amount_bxsec(&mut self, candy_addr: Address, candy_amnt: Decimal) -> Decimal {   
            let price = CandyDex::candyprice(self, candy_addr);
        ...
        pub fn get_candy_buy_amount_bcsex(&mut self, candy_addr: Address, xrd_amnt: Decimal) -> Decimal {    
            let price = CandyDex::candyprice(self, candy_addr);
        ...
        pub fn get_xrd_sell_amount_becsx(&mut self, candy_addr: Address, candy_amnt: Decimal) -> Decimal {   
        ...
        pub fn get_candy_buy_amount_bcsec(&mut self, addr_in: Address, amnt_out: Decimal, addr_out: Address) -> Decimal {  
            ...
            // Obtain a minimum candy amount in exchange of an exact XRD amount. Function swap exact XRD for candy.
        pub fn buy_candy_sell_exact_xrd(&mut self, min_in: Decimal, addr_in: Address, xrd_out: Bucket) -> Bucket { 
            ...
            // Obtain a minimum candy amount in exchange of an exact candy amount. Function swap exact candy for candy.
        pub fn buy_candy_sell_exact_candy(&mut self, min_in: Decimal, addr_in: Address, candy_out: Bucket) -> Bucket { 
            ...
            // Obtain a minimum XRD amount in exchange of an exact candy amount. Function swap exact candy for XRD.
        pub fn buy_xrd_sell_exact_candy(&mut self, xrd_min: Decimal, candy_out: Bucket) -> Bucket {  
            ...
            // Obtain an exact candy amount in exchange of a maximum XRD amount. Function swap XRD for exact candy.    
        pub fn buy_exact_candy_sell_xrd(&mut self, nbr_in: Decimal, addr_in: Address, xrd_out: Bucket) -> (Bucket,Bucket) { 
            ...
            // Obtain an exact candy amount in exchange of a maximum candy amount. Function swap candy for exact candy.
        pub fn buy_exact_candy_sell_candy(&mut self, amnt_in: Decimal, addr_in: Address, candy_out: Bucket) -> (Bucket,Bucket) {
            let addr_out = candy_out.resource_address();
            assert!(addr_in != addr_out," Same candy's address detect! ");
            let amount_in = CandyDex::adjust_fee(self, amnt_in);
            let candy_output: Bucket = CandyDex::candyput_sec(self, amount_in, addr_in, 0i32.into(), 1i32.into(), candy_out);
            let candy_bucket: Bucket = CandyDex::candytake(self, amount_in, addr_in);
            (candy_output,candy_bucket)
        }  
            // Obtain an exact XRD amount in exchange of a maximum candy amount. Function swap candy for exact XRD.
        pub fn buy_exact_xrd_sell_candy(&mut self, xrd_in: Decimal, candy_out: Bucket) -> (Bucket,Bucket) {         
            ...
            let new_price: Decimal = CandyDex::price_mod(self, xrd_input/price_out, addr, price_out, 0i32.into()); 
            let (name,symbol,_price) = self.candy_map.get(&addr).unwrap();
            let name_str = name.to_string();
            let symbol_str = symbol.to_string();
            self.candy_map.insert(addr,(name_str,symbol_str,new_price));
            let candy_output: Bucket = CandyDex::candyput_sec(self, xrd_input, addr, 1i32.into(), new_price, candy_out);             
        ...
        pub fn flashswap(&mut self, amnt_in: Decimal, addr_in: Address, bckt: Bucket, ext_addr: Address, method: String) -> (Bucket,Bucket) {
            ...
            if  addr_in == bckt.resource_address() { 
                ...
                } else { token_bucket = CandyDex::candytake(self, amount_in, addr_in); }    
            } else if addr_in == self.collected_xrd.resource_address() && addr_in != bckt.resource_address() {
                token_bucket = self.collected_xrd.take(*&(amount_in-amount_in*self.fee/100));
                self.xrd_fee = self.xrd_fee+amount_in*self.fee/100;
                price_in = 1i32.into();
                price_out = CandyDex::candyprice(self, bckt.resource_address());
            } else if addr_in != bckt.resource_address() && bckt.resource_address() == self.collected_xrd.resource_address() {
                token_bucket = CandyDex::candytake(self, amount_in, addr_in);
                price_in = CandyDex::candyprice(self, addr_in); 
                price_out = 1i32.into(); 
            } else if bckt.resource_address() != self.collected_xrd.resource_address() {
                token_bucket = CandyDex::candytake(self, amount_in, addr_in);
                price_in = CandyDex::candyprice(self, addr_in);                      
                price_out = CandyDex::candyprice(self, bckt.resource_address()); 
            } else { info!(" Check out addresses! "); std::process::abort(); }
            ...
            if bckt.resource_address() != self.collected_xrd.resource_address() { 
                let v = self.candy_vaults.entry(bckt.resource_address()).or_insert(Vault::new(bckt.resource_address()));
                v.put(token_return);       
                token_output = v.take(amount-nmbr); 
            } else { self.collected_xrd.put(token_return);                      
...
// Build a structure and implement it to populate a meta hashmap and relate metaCandy resource with respective Candy resource.         
