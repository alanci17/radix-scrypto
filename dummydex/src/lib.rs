...
blueprint! {
    struct DummyDex {
        flash_xrd: Vault,
        flash_vault: HashMap<ResourceAddress, Vault>,
        flash_price: HashMap<ResourceAddress, Decimal>
    }
    ...
    impl DummyDex {
        pub fn new() -> ComponentAddress {
            ...
            let mut dummy_dex: DummyDexComponent = Self {
                flash_xrd: Vault::new(RADIX_TOKEN),
                flash_vault: HashMap::new(),
                flash_price: HashMap::new()
            }
            .instantiate();
            dummy_dex.add_access_check(access_rules);
        ...
        pub fn arb_dex(&mut self, token: Bucket, token_out_addr: ResourceAddress ) -> Bucket {
                let token_bucket;
            ...
            fn flashprice(&mut self, candy_out_addr: ResourceAddress) -> Decimal {
                let price: Decimal;  
